...
import AddSM from "./addSM.vue";
import EditSM from "./editSM.vue";
export default {
  name: "social_media",
  components: {
    AddSM,
  ...
  async created() {
    ...
    if (this.identity.role == "colaborador") {
      this.getInfoByIdSM({
        ...
        option: "social_media/colaborador",
      });
    } else {
  ...
  computed: {
    ...
    data() {
      let data = this.$store.getters["social_media/data"];
  ...
  methods: {
    ...
    changeView(option, item) {
      this.item = item;
      this.setAddedSM(item);
      this.setHistoryOptionSM(option);
    },
    ...
    eliminar: async function () {
      ...
      let result = await this.$store.dispatch("social_media/deleteItemSM", {
        option: "social_media",
        id: id,
      });
