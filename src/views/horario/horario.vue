    ...
    <div v-if="historyOption == 'Default'">
      ...
      <div class="tbl_usuarios">
        <div class="tbl_header dflx">
          <div class="tblh tblrad width2">
            <p>DATOS DE HORARIO</p>
          </div> 
          <div class="tblh2">
            <p>EDITAR</p>
          </div>
        ...
        <div tbl_content>
          <div v-if="data.length > 0">
            <div
              ...
              :key="key"
            >
              <div class="tbl_item width2">
                <p><b>Día(s):</b> {{ item.start_day + ' '+item.end_day  }}</p> 
                <p><b>Horario:</b> {{ item.start_hour + ' - '+item.end_hour  }}</p> 
              </div> 
...
import AddHor from "./addHor.vue";
import EditHor from "./editHor.vue";
export default {
  name: "horario",
  components: {
    AddHor,
  ...
  methods: {
    ...
    changeView(option, item) {
      this.item = item;
      this.setAddedHor(item);
      this.setHistoryOptionHor(option);
    },
    ...
    eliminar: async function () {
      ...
      let result = await this.$store.dispatch("horario/deleteItemHor", {
        option: "horario",
        id: id,
      });
