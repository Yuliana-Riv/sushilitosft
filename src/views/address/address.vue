          ...
          <div v-if="data.length > 0">
            ...
            >
              <div class="tbl_item width2">
                <p>{{ item.address + ' '+item.colonia  }}</p>
                <p>{{ item.zip + ', '+item.city + ', '+item.state + ', '+item.country  }}</p>
              </div> 
...
import AddAdr from "./addAdr.vue";
import EditAdr from "./editAdr.vue";
export default {
  name: "address",
  components: {
    AddAdr,
  ...
  methods: {
    ...
    changeView(option, item) {
      this.item = item;
      this.setAddedAdr(item);
      this.setHistoryOptionAdr(option);
    },
    ...
    eliminar: async function () {
      ...
      let result = await this.$store.dispatch("address/deleteItemAdr", {
        option: "address",
        id: id,
      });
