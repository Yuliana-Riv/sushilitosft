...
export default {
    ...
    actions:{
        setAddedNv({commit}, payload){
            commit(SET_ADDED, payload);  
        },

        setHistoryOptionNv({commit}, payload){
            commit(SET_HISTORY_OPTION, payload);  
        },
        async getAllInfoNv({ commit }, payload) {
            commit(GETTING_INFO);
            commit(GETTING_MSG);
            //console.log(payload)
        ...
        async getInfoByIdNv({ commit }, payload) {
            commit(GETTING_INFO);
            try {              
                let token = userService.getToken() 
                const _pk = ncryptService._createPubToken()
        ...
        async deleteItemNv({ commit }, payload) { 
        ...
        async editItemNv({ commit }, payload) { 
            commit(SET_STATUS, 'SUCCESS')
            try {               
                let token = userService.getToken() 
                const _pk = ncryptService._createPubToken()
        ...
        async addItemNv({ commit }, payload) { 
            commit(SET_STATUS, 'SUCCESS')
